  ...
  return isLoading ? "" : error ? <Message variant='bg-red-500' /> : (
    <div className="max-w-full mx-auto relative">
      <div id="default-carousel" className="relative" data-carousel="static">
          ...
          {products.map((product, index) => (
            ...
            >
              <Link to={`/product/${product._id}`}>
                <img src={product.image} className="w-1/2 h-96" alt={product.name} />
              </Link>
              <div className="absolute bottom-0 left-0 right-0 bg-gray-800/30 p-2">
        ...
        >
          <span className="inline-flex flex-col h-full justify-center hover:border hover:border-gray-800/50">
            <MdArrowRight className='text-5xl text-gray-900' />
